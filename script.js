...
const cinemaData = {
  ...
  BOGOR: ["Botani Square ", "Bogor Trade Mall", "Cibinong City Mall"],
  DEPOK: ["Depok Town Square", "Margo City ", "Pesona Square "],
  TANGERANG: [
    "AEON Mall BSD City ",
  ...
  .addEventListener("submit", function (event) {
    ...
    if (validateLogin(email, password)) {
      ...
      if (loginAttempts >= 3) {
        ...
        loginFailedPopup.querySelector("p").textContent =
          "Invalid Email or Password"; // Ubah pesan error
...
document.addEventListener("DOMContentLoaded", function () {
  fetch("movies.json")
    ...
    .then((data) => {
      displayMovies(data.nowPlaying, "now-playing-movies");
      displayMovies(data.upcoming, "upcoming-movies");
    })
...
function displayMovies(movies, elementId) {
  ...
  movies.forEach((movie) => {
    ...
    movieElement.innerHTML = `
            <a href="details.html?title=${encodeURIComponent(movie.title)}">
